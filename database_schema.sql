...
DELIMITER // CREATE PROCEDURE AddFullNameToUsers() BEGIN IF NOT EXISTS (
    SELECT *
...
DELIMITER // -- [TRIGGER 1] 전표 상세 입력 후 재고 자동 반영
CREATE TRIGGER after_trade_detail_insert
AFTER
INSERT ON trade_details FOR EACH ROW BEGIN
...
SELECT trade_type,
    trade_date,
    company_id,
    warehouse_id INTO v_trade_type,
    v_trade_date,
    v_company_id,
    v_warehouse_id
FROM trade_masters
WHERE id = NEW.trade_master_id;
...
SET v_before_qty = 0;
END IF;
IF v_trade_type = 'PURCHASE' THEN -- [NEW] 신규 순번 계산: 현재 최대 순번 + 1
...
IF v_after_qty < 0 THEN SIGNAL SQLSTATE '45000'
...
END IF;
END // -- [TRIGGER 2] 전표 상세 삭제 전 재고 원상복구
...
FROM trade_masters
WHERE id = OLD.trade_master_id;
IF v_trade_type = 'PURCHASE' THEN -- 매칭된 내역이 있는지 확인
...
IF v_matched_count > 0 THEN SIGNAL SQLSTATE '45000'
...
WHERE trade_detail_id = OLD.id;
END IF;
IF v_trade_type = 'SALE' THEN -- Aggregate Inventory 복구 (매출 취소이므로 재고 증가)
UPDATE inventory
SET quantity = quantity + OLD.quantity,
    weight = weight + IFNULL(OLD.total_weight, 0)
WHERE product_id = OLD.product_id;
...
VALUES -- Basic Info
    ('COMPANY_LIST', 'READ', '거래처 목록 조회'),
    ('COMPANY_LIST', 'CREATE', '거래처 등록'),
    ('COMPANY_LIST', 'UPDATE', '거래처 수정'),
    ('COMPANY_LIST', 'DELETE', '거래처 삭제'),
    ('PRODUCT_LIST', 'READ', '품목 목록 조회'),
    ('PRODUCT_LIST', 'CREATE', '품목 등록'),
    ('PRODUCT_LIST', 'UPDATE', '품목 수정'),
    ('PRODUCT_LIST', 'DELETE', '품목 삭제'),
    ('WAREHOUSES', 'READ', '창고 조회'),
    ('WAREHOUSES', 'CREATE', '창고 등록'),
    ('WAREHOUSES', 'UPDATE', '창고 수정'),
    ('WAREHOUSES', 'DELETE', '창고 삭제'),
    ('PAYMENT_METHODS', 'READ', '결제수단 조회'),
    ('PAYMENT_METHODS', 'CREATE', '결제수단 등록'),
    ('PAYMENT_METHODS', 'UPDATE', '결제수단 수정'),
    ('PAYMENT_METHODS', 'DELETE', '결제수단 삭제'),
    ('EXPENSE_CATEGORIES', 'READ', '지출항목 조회'),
    ('EXPENSE_CATEGORIES', 'CREATE', '지출항목 등록'),
    ('EXPENSE_CATEGORIES', 'UPDATE', '지출항목 수정'),
    ('EXPENSE_CATEGORIES', 'DELETE', '지출항목 삭제'),
    -- Trades
    ('TRADE_LIST', 'READ', '전표 목록 조회'),
    ('TRADE_LIST', 'CREATE', '전표 등록'),
    ('TRADE_LIST', 'UPDATE', '전표 수정'),
    ('TRADE_LIST', 'DELETE', '전표 삭제'),
    ('PURCHASE', 'READ', '매입 전표 조회'),
    ('PURCHASE', 'CREATE', '매입 전표 등록'),
    ('SALE', 'READ', '매출 전표 조회'),
    ('SALE', 'CREATE', '매출 전표 등록'),
    -- Auction
    ('AUCTION_IMPORT', 'READ', '경매 데이터 조회'),
    ('AUCTION_IMPORT', 'CREATE', '경매 데이터 가져오기'),
    ('AUCTION_IMPORT', 'UPDATE', '경매 데이터 수정'),
    ('AUCTION_IMPORT', 'DELETE', '경매 데이터 삭제'),
    ('AUCTION_ACCOUNTS', 'READ', '경매 계정 조회'),
    ('AUCTION_ACCOUNTS', 'CREATE', '경매 계정 등록'),
    ('AUCTION_ACCOUNTS', 'UPDATE', '경매 계정 수정'),
    ('AUCTION_ACCOUNTS', 'DELETE', '경매 계정 삭제'),
    -- Inventory
    ('INVENTORY_LIST', 'READ', '재고 조회'),
    ('INVENTORY_QUICK', 'READ', '재고 수정(Quick)'),
    ('INVENTORY_QUICK', 'UPDATE', '재고 수정'),
    ('INVENTORY_TRANSFER', 'READ', '재고 이동 조회'),
    ('INVENTORY_TRANSFER', 'CREATE', '재고 이동 등록'),
    ('INVENTORY_PRODUCTION', 'READ', '재고 작업 조회'),
    ('INVENTORY_PRODUCTION', 'CREATE', '재고 작업 등록'),
    (
        'INVENTORY_PRODUCTION_HISTORY',
        'READ',
        '재고 작업 이력 조회'
    ),
    ('MATCHING', 'READ', '매칭 관리 조회'),
    ('MATCHING', 'UPDATE', '매칭 실행'),
    ('INVENTORY_HISTORY', 'READ', '재고 이력 조회'),
    ('INVENTORY_AUDIT', 'READ', '재고 실사 조회'),
    ('INVENTORY_AUDIT', 'CREATE', '재고 실사 등록'),
    ('INVENTORY_AUDIT', 'UPDATE', '재고 실사 수정'),
    ('INVENTORY_AUDIT', 'DELETE', '재고 실사 삭제'),
    -- Payment
    ('COMPANY_BALANCES', 'READ', '거래처 잔고 조회'),
    ('EXPENSES', 'READ', '지출 내역 조회'),
    ('EXPENSES', 'CREATE', '지출 등록'),
    ('EXPENSES', 'UPDATE', '지출 수정'),
    ('EXPENSES', 'DELETE', '지출 삭제'),
    -- Management
    ...
    -- Settings
    ('SETTINGS', 'READ', '시스템 설정 조회'),
    ('SETTINGS', 'UPDATE', '시스템 설정 수정'),
    ('BACKUP_SYSTEM', 'READ', '백업 관리 조회'),
    ('BACKUP_SYSTEM', 'CREATE', '백업 생성'),
    ('COMPANY_INFO', 'READ', '본사 정보 조회'),
    ('COMPANY_INFO', 'UPDATE', '본사 정보 수정'),
    ('USER_MANAGEMENT', 'READ', '사용자 관리 조회'),
    ('USER_MANAGEMENT', 'CREATE', '사용자 등록'),
    ('USER_MANAGEMENT', 'UPDATE', '사용자 수정'),
    ('USER_MANAGEMENT', 'DELETE', '사용자 삭제'),
    ('ROLE_MANAGEMENT', 'READ', '권한 관리 조회'),
    ('ROLE_MANAGEMENT', 'CREATE', '권한항목 등록'),
    ('ROLE_MANAGEMENT', 'UPDATE', '권한항목 수정'),
    ('ROLE_MANAGEMENT', 'DELETE', '권한항목 삭제'),
...
SELECT 'admin',
    '$2b$10$Ap2OlDUGfAw6BU0DXnEfjeB1WFISDMY7KMFQeaCCpSjuCRIfn.kOO6',
    'admin',
    '관리자',
    id
FROM roles
WHERE name = 'Administrator' ON DUPLICATE KEY
...
UPDATE users
SET password_hash = '$2b$10$Ap2OlDUGfAw6BU0DXnEfjeB1WFISDMY7KMFQeaCCpSjuCRIfn.kOO6'
...
-- 3.2 WAREHOUSES (창고)
INSERT IGNORE INTO warehouses (name, type, is_default, is_active, display_order)
VALUES ('메인 냉동고', 'MAIN', 1, 1, 1),
    ('외부 창고', 'STORAGE', 0, 1, 2),
    ('보관 창고', 'STORAGE', 0, 1, 3);
-- 3.3 EXPENSE CATEGORIES (지출 항목)
INSERT IGNORE INTO expense_categories (name, is_active, sort_order)
VALUES ('식비', 1, 1),
    ('차량유지비', 1, 2),
    ('소모품비', 1, 3),
    ('임차료', 1, 4),
...
VALUES (
        ...
        '서울시 송파구 가락동',
        '02-1234-5678',
        'admin@hongdafood.com',
        '기초 설정 정보입니다. 수정하여 사용하세요.'
    );