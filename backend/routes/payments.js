...
router.get('/company-today-summary/:companyId', async (req, res) => {
  try {
    ...
    // GROUP BY 제거하고 시간순 정렬
    const [paymentDetails] = await pool.query(
      `SELECT 
          id,
          transaction_date,
          IFNULL(payment_method, '미지정') as payment_method, 
          amount,
          notes
       FROM payment_transactions
       WHERE company_id = ?
    ...
    // 호환성을 위해 cash, bank 별도 계산 (필요한 경우)
    let cashPayment = 0;
    let bankPayment = 0;

    paymentDetails.forEach(p => {
      if (p.payment_method === '현금' || p.payment_method === 'CASH') {
        cashPayment += parseFloat(p.amount);
      } else if (p.payment_method === '계좌이체' || p.payment_method === 'BANK' || p.payment_method === '통장') {
        bankPayment += parseFloat(p.amount);
      }
    });
